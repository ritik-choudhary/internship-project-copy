...
function ItineraryModalComponent(props) {
  ...
  useEffect(() => {
    if (isEditing || isSharing) {
      ...
      const selectedItinerary = selectedResource.itineraryList.find(
        (item) => item.id === param.itineraryID
      )
      setItineraryToEdit(selectedItinerary)
      setTitle(selectedItinerary.title)
      setCreatedOn(selectedItinerary.createdOn)
      setCreatedBy(selectedItinerary.createdBy)
      setTextNote(selectedItinerary.note)
    }
  }, [
    isEditing,
    isSharing,
