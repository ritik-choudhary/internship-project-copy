...
export default function TopicInformtaionModal(props) {
  return (
    <WorkspaceConsumer>
      {(value) => {
        return (
          <TopicInformationModalComponent
            value={value}
            {...props}
          ></TopicInformationModalComponent>
        )
...
function TopicInformationModalComponent(props) {
  ...
  const [topicInformationToEdit, setTopicInformationToEdit] = useState()
  ...
  useEffect(() => {
    if (isEditing) {
      ...
      const selectedTopicInformation = selectedResource.topicInformations.find(
        (item) => item.id === param.topicInformationID
      )
      setTopicInformationToEdit(selectedTopicInformation)
      setTitle(selectedTopicInformation.title)
      setCreatedOn(selectedTopicInformation.createdOn)
      setCreatedBy(selectedTopicInformation.createdBy)
      setTextNote(selectedTopicInformation.note)
    }
  }, [
    isEditing,
    param.id,
  ...
  return (
    ...
    >
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          if (isEditing) {
            ...
            value.editTopicInformation(
              param.id,
              param.spaceKey,
              param.workshopID,
              param.resourceID,
              topicInformationToEdit.id,
              topicInformation
            )
          } else {
            const topicInformation = {
      ...
      >
        <div
          className='topic-information-name'
